...
Available on `tag` and `write-pdfs` (and as a checkbox in the GUI's Tag/Write PDFs tabs). Auto-hyperlinks in-text page and chapter references (e.g. "see p. 208") before metadata is written, using a separate GURPS-specific sibling script on your machine (see `gurps_hyperlink.py`) — set `gurps_hyperlink_script` in `config.yaml` to that script's path first, or this is a no-op with a clear log line rather than a crash. Off by default. This only makes sense for GURPS PDFs; it's a machine-specific integration, not a general feature of this project.
