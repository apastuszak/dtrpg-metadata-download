...
With `--root`, before the per-file loop starts you're asked once for a series name, with a hint that you can *"leave blank if all books are not in the same series."* Give a name and it's applied to every book written in that run with no further prompting (the series field is simply left out of every later screen); leave it blank and you're asked per book instead. (Files matched via `dtrpg_urls.csv`, below, never get a series screen either way, since that path is deliberately non-interactive end to end.)
